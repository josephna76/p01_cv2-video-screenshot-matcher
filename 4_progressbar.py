...
def match_screenshot_to_video(screenshot_path, video_path):
    ...
    for _ in tqdm(
        range(total_frames),
        desc=f"Frames in {os.path.basename(video_path)}",
        unit="frame",
        leave=False,
    ):
        ...
        res = cv2.matchTemplate(resized_frame, screenshot, cv2.TM_CCOEFF_NORMED)
...
with open("matches.txt", "w") as f:
    for screenshot in tqdm(
        screenshots, desc="Processing screenshots", unit="screenshot"
    ):
        best_match_video = None
        best_score = 0.0

        for video in tqdm(
            videos, desc="Comparing with videos", unit="video", leave=False
        ):
